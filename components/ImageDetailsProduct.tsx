    ...
    <>
      ...
      {data.length !== 1 && (
        <Swiper
          ...
          slidesPerView={3}
          modules={[FreeMode, Navigation, Thumbs]}
          className="flex justify-center mt-2 md:max-w-[495px]"
          breakpoints={{
            640: {
              slidesPerView: 3,
              spaceBetween: 20,
          ...
          <div className="flex justify-center">
            {data.map(item => (
              <SwiperSlide key={item.id} className="flex justify-center">
                <Image src={`${process.env.NEXT_PUBLIC_API_BASE_URL}${item.imageUrl}`} alt={item.title || ''} width={100} height={100} className="m-auto" />
              </SwiperSlide>
            ))}
          </div>
        </Swiper>
